...
\begin{equation*}
	h[m, n] = \sum_{k,l}f[k,l]I[m+k, n+l] \text{2d correlation} \\
	h[m, n] = \sum_{k,l}f[k,l]I[m-k, n-l] \text{2d convolution}
\end{equation*}

\item As long as the filter is nonsymmetric, the output between convolution and correlation is different. Here I use the '../data/fish.bmp' as an example. The input image, filter and output images are shown  below. The code to obtain the results is attahced at last page. \\
First I read the image into grayscale for sake of simplicity. As brightness in image increases, the response in output will increase. Overall brighter regions will give higher correlation response. To avoid this, the mean is substracted for both input image and filter. \\
...
\begin{figure*}[htbp]
	\centering
	\subfloat[\centering fish input image (scaled to 0.25)]{{\includegraphics[scale=0.25]{fish.png}}} %
	\qquad
	\subfloat[\centering filter image]{{\includegraphics[]{filter_fish.png}}} %
	\qquad
	\subfloat[\centering Correlate2d output image (scaled to 0.25)]{{\includegraphics[scale=0.25]{correlate2d_fish.png}}}
	\qquad
	\subfloat[\centering Convolve2d output image (scaled to 0.25)]{{\includegraphics[scale=0.25]{convolve2d_fish.png}}}
...
Below is the figure for excution time plot for convolution operation with different size of kernels and images. We can see that with the filter sizes and image sizes increaseing, the computation time increases as well.
